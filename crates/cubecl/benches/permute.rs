...
fn bench_permute<R: Runtime, E: frontend::Float + CubeElement>(
    ...
    let dtype = E::as_type_native().unwrap();
    let input = TensorHandle::<R>::new_contiguous(input_shape.clone(), handle, dtype);

    let output_shape: Vec<usize> = axes.iter().map(|&i| input_shape[i]).collect();
    let output = TensorHandle::<R>::empty(&client, output_shape, dtype);
...
fn run_benchmark_suite<R: Runtime>(device: R::Device, backend_name: &str) {
    ...
    // : Channel shuffle NCHW → NHWC [0,2,3,1]
    ...
    // : Attention transpose [B, H, N, D] → [B, N, H, D] [0,2,1,3]
