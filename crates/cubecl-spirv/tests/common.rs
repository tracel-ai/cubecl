...
#[allow(unused)]
pub fn array_vec(tensor: &Handle, vectorization: u8) -> ArrayArg<'_, WgpuRuntime> {
    unsafe { ArrayArg::from_raw_parts(tensor, 1, vectorization) }
}
...
pub fn as_bytes(module: &Module) -> Vec<u8> {
    let words = module.assemble();
    words.into_iter().flat_map(|it| it.to_le_bytes()).collect()
}
