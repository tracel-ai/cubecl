...
use crate::{
    ...
    new_ir::{
        element::Container, flatten::flatten_expr, Expr, Expression, SquareType, Strided,
        Vectorization,
    },
...
pub fn flatten_cmma_expr(expr: CmmaExpression, context: &mut CubeContext) -> Option<ExpandElement> {
    let res = match expr {
        CmmaExpression::Init {
            ident,
            m,
        ...
        CmmaExpression::Fill { matrix, value } => {
            let value = flatten_expr(*value, context).unwrap();
            let matrix = flatten_expr(*matrix, context).unwrap();
        ...
        CmmaExpression::Load {
            ...
            let stride = flatten_expr(*stride, context).unwrap();
            let values = flatten_expr(*values, context).unwrap();
            let matrix = flatten_expr(*matrix, context).unwrap();
        ...
        CmmaExpression::Store {
            ...
            let stride = flatten_expr(*stride, context).unwrap();
            let out = flatten_expr(*out, context).unwrap();
            let matrix = flatten_expr(*matrix, context).unwrap();
        ...
        CmmaExpression::Execute {
            ...
            let mat_a = flatten_expr(*mat_a, context).unwrap();
            let mat_b = flatten_expr(*mat_b, context).unwrap();
            let mat_c = flatten_expr(*mat_c, context).unwrap();
            let mat_d = flatten_expr(*mat_d, context).unwrap();
