...
fn require_complex_usage(
    ...
    // Device properties are populated for all production paths (kernel launch
    ...
    let properties = scope.state().device_properties.clone();
...
pub(crate) fn validate_complex_operation(scope: &Scope, operation: &Operation) {
    match operation {
        Operation::Arithmetic(arithmetic) => match arithmetic {
            ...
            Arithmetic::Rem(op) => reject_complex_operation(scope, [op.lhs.ty, op.rhs.ty], "%"),
            Arithmetic::ModFloor(op) => {
                reject_complex_operation(scope, [op.lhs.ty, op.rhs.ty], "mod_floor")
            }
...
pub(crate) fn validate_complex_assign_operation(scope: &Scope, operation: &Operation) {
    match operation {
        ...
        Operation::Arithmetic(Arithmetic::Rem(op)) => {
            reject_complex_operation(scope, [op.lhs.ty, op.rhs.ty], "%=")
        }
        Operation::Arithmetic(Arithmetic::ModFloor(op)) => {
            reject_complex_operation(scope, [op.lhs.ty, op.rhs.ty], "mod_floor=")
        }
        _ => {}
    }
