...
pub trait ComplexAbs:
    ComplexCore + CubePrimitive<Scalar: ComplexAbsNativeExpand<FloatElem = Self::FloatElem>>
{
    fn abs(self) -> Self::FloatElem {
        unexpanded!()
    }

    fn norm(self) -> Self::FloatElem {
        unexpanded!()
    }

    fn __expand_abs(scope: &Scope, x: NativeExpand<Self>) -> NativeExpand<Self::FloatElem> {
        x.__expand_abs_method(scope)
    }

    fn __expand_norm(scope: &Scope, x: NativeExpand<Self>) -> NativeExpand<Self::FloatElem> {
        x.__expand_norm_method(scope)
    }
}

pub trait ComplexMath: ComplexAbs + Exp + Log + Sin + Cos + Sqrt + Tanh + Powf {}

pub trait ComplexAbsExpand {
    type FloatElem: Scalar;
    fn __expand_abs_method(self, scope: &Scope) -> NativeExpand<Self::FloatElem>;
    fn __expand_norm_method(self, scope: &Scope) -> NativeExpand<Self::FloatElem>;
}

pub trait ComplexAbsNativeExpand {
    type FloatElem: Scalar;
    fn __expand_native_abs(scope: &Scope, input: ExpandValue) -> ExpandValue;
}

impl<T: ComplexAbs> ComplexAbsExpand for NativeExpand<T> {
    type FloatElem = T::FloatElem;

    fn __expand_abs_method(self, scope: &Scope) -> NativeExpand<T::FloatElem> {
        T::Scalar::__expand_native_abs(scope, self.into()).into()
    }

    fn __expand_norm_method(self, scope: &Scope) -> NativeExpand<T::FloatElem> {
        T::Scalar::__expand_native_abs(scope, self.into()).into()
    }
}
...
macro_rules! impl_complex {
    ($primitive:ty, $kind:ident, $float:ty, $ir_ty:ty, $float_kind:ident) => {
        ...
        impl ComplexAbs for $primitive {}
        impl ComplexAbsNativeExpand for $primitive {
            type FloatElem = $float;
            fn __expand_native_abs(scope: &Scope, input: ExpandValue) -> ExpandValue {
