...
pub fn test_kernel_different_rank_first_biggest<R: Runtime>(client: ComputeClient<R::Server, R::Channel>) {
    let vectorisation = 2;
...
pub fn test_kernel_different_rank_last_biggest<R: Runtime>(client: ComputeClient<R::Server, R::Channel>) {
    let vectorisation = 2;
...
macro_rules! testgen_different_rank {
    () => {
        ...
        fn test_kernel_different_rank_first_biggest() {
            ...
            cubecl_core::runtime_tests::different_rank::test_kernel_different_rank_first_biggest::<TestRuntime>(
                client,
            );
        }
        ...
        fn test_kernel_different_rank_last_biggest() {
            let client = TestRuntime::client(&Default::default());
            cubecl_core::runtime_tests::different_rank::test_kernel_different_rank_last_biggest::<TestRuntime>(
                client,
            );
        }
    };
