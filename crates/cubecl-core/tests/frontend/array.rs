...
mod tests {
    ...
    use cubecl_core::{
        cpa,
        ir::{self, Elem, Item, UIntKind, Variable, VariableKind},
    };
    ...
    fn array_add_assign() {
        let mut context = CubeContext::default();
        let array = context.input(0, Item::new(Elem::UInt(UIntKind::U32)));
    ...
    fn inline_macro_array_add_assign_simple() -> Vec<ir::Instruction> {
        ...
        let mut scope = context.into_scope();
        let local = scope.create_local(Item::new(Elem::UInt(UIntKind::U32)));
