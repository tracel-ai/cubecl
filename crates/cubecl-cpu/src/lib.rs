...
mod tests {
    ...
    #[test]
    fn a_cleanup_keeps_freed_storage_mapped_until_the_queued_launches_run() {
        let client = TestRuntime::client(&Default::default());
        ...
        let input = client_a.create_from_slice(u32::as_bytes(&[7, 7]));
        // On the other stream, so the input is alone on its page and the cleanup frees it.
        let output = client_b.empty(core::mem::size_of::<u32>());

        // A full fresh batch reaches the pool without a wait, which releases the input at enqueue.
