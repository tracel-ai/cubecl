    ...
    LowerBuiltins<T>: MatchRewrite,
{
    fn compile_ir(
        ...
        // Emit here, on the `Result` path, rather than lazily from `Display`. An op that
        // survives lowering with no `OpToCPP` impl — a target-gated lowering with no fallback,
        // say — can then fail the compilation instead of panicking on this thread, which the
        // launching thread never observes.
