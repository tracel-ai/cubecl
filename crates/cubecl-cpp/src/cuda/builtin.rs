...
impl CudaBuiltin for Builtin {
    fn display_cuda(&self, ctx: &Context) -> String {
        ...
        match self {
            Builtin::CubePosCluster if clusters => "cluster.block_rank()".into(),
            Builtin::CubePosClusterX if clusters => "cluster.block_index().x".into(),
            Builtin::CubePosClusterY if clusters => "cluster.block_index().y".into(),
            Builtin::CubePosClusterZ if clusters => "cluster.block_index().z".into(),
