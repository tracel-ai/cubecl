...
mod tests {
    ...
    #[test]
    fn the_base_name_drops_the_type_mangling() {
        assert_eq!(intrinsic_base("llvm.sinh.f32"), Some("sinh"));
        assert_eq!(intrinsic_base("llvm.sinh.v4f32"), Some("sinh"));
        assert_eq!(intrinsic_base("llvm.powi.f32.i32"), Some("powi"));
        assert_eq!(intrinsic_base("__ocml_sinh_f32"), None);
    }

    /// A module the hardware can already compile reports needing nothing, so that a machine
    /// without `ROCm` installed still compiles the kernels that never call into it.
    #[test]
    fn a_module_of_hardware_arithmetic_needs_no_device_libs() {
        for base in ["sqrt", "fabs", "floor", "exp", "sin"] {
            assert!(!needs_ocml(base, "f32"), "{base} would pull in OCML");
        }
    }
    ...
    fn only_what_the_backend_cannot_lower_goes_to_ocml() {
        for base in [
            "sinh", "cosh", "tanh", "tan", "asin", "acos", "atan", "atan2",
        ] {
        ...
        for base in ["sqrt", "fabs", "floor", "ceil", "trunc", "roundeven"] {
            assert!(!needs_ocml(base, "f32"), "{base} is hardware");
            assert!(!needs_ocml(base, "f64"), "{base} is hardware");
        }
    }
}
