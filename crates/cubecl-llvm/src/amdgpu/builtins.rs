...
mod tests {
    ...
    /// Every positional builtin the hardware provides must have an intrinsic, and it must be
    /// the intrinsic for the right axis: this checks the exact pairing, not just that every
    /// builtin is present somewhere in the table, so a swapped axis (e.g.
    /// `llvm.amdgcn.workitem.id.y` mapped to `Builtin::UnitPosZ`) is caught here rather than
    /// producing a silently transposed launch grid.
    #[test]
    fn intrinsic_tables_map_each_axis_to_the_right_builtin() {
        let expected = [
            ("llvm.amdgcn.workitem.id.x", Builtin::UnitPosX),
            ("llvm.amdgcn.workitem.id.y", Builtin::UnitPosY),
            ("llvm.amdgcn.workitem.id.z", Builtin::UnitPosZ),
        ...
        let table: Vec<(&str, Builtin)> = WORKITEM_ID.into_iter().chain(WORKGROUP_ID).collect();
        assert_eq!(
            table.len(),
            expected.len(),
            "intrinsic tables should contain exactly the expected number of entries"
        );
        for (intrinsic, builtin) in expected {
            assert!(
                table.contains(&(intrinsic, builtin)),
                "{intrinsic} should map to {builtin:?}"
            );
        }
    }

    /// RDNA runs wave32, CDNA and GCN wave64. Getting this wrong makes
    /// `PlaneDim` silently wrong in every kernel that reads it.
    #[test]
    fn plane_dim_follows_the_architecture() {
        assert_eq!(crate::amdgpu::plane_dim_for("gfx1201"), 32);
        assert_eq!(crate::amdgpu::plane_dim_for("gfx1100"), 32);
        assert_eq!(crate::amdgpu::plane_dim_for("gfx1036"), 32);
        assert_eq!(crate::amdgpu::plane_dim_for("gfx942"), 64);
        assert_eq!(crate::amdgpu::plane_dim_for("gfx90a"), 64);
    }
}
