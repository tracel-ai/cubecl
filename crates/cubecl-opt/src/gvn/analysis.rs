...
use cubecl_ir::{ExpandValue, ValueKind};
...
impl GvnState {
    ...
    /// values that represent them are also available there.
    fn build_block_sets_forward(
        &mut self,
        func: &mut Function,
        block: NodeIndex,
        mut leaders: HashMap<u32, ExpandValue>,
        tmp_gen: HashSet<ExpandValue>,
    ) -> (HashMap<u32, ExpandValue>, HashSet<ExpandValue>) {
        // Expressions generated (used on the right hand side of an instruction) in this block
        let mut exp_gen = LinkedList::new();
        // Values generated by the output values of phi nodes in this block.
        let mut phi_gen = HashMap::new();
        // Temporaries/values that are generated with a volatile expression on the right hand
...
pub fn phi_translate(
    func: &Function,
    phi_gen: &HashMap<u32, ExpandValue>,
    ...
    // Translate each phi's output value to the input value
