    ...
    fn read_async(
        ...
        fn inner(
            ...
            for descriptor in descriptors {
                ...
                if shape.len() <= 1 || is_contiguous(shape, strides) {
                    server.copy_from_binding(binding, &mut data);
                } else if let Some(pitch_elems) = row_pitch_elems(shape, strides) {
                    server.copy_from_binding_2d(binding, &mut data, shape, pitch_elems, elem_size);
                }
