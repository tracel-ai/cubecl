...
pub fn slice_assign() {
    ...
    let kernel = slice_assign_kernel::create_dummy_kernel::<HipRuntime>(
        CubeCount::Static(1, 1, 1),
        CubeDim::new(1, 1, 1),
        tensor(&input),
        tensor(&output),
    );
    let expected = include_str!("slice_assign.cu.hip").replace("\r\n", "\n");
...
pub fn subcube_sum() {
    ...
    let kernel = kernel_sum::create_dummy_kernel::<HipRuntime>(
        CubeCount::Static(1, 1, 1),
        CubeDim::new(4, 1, 1),
        tensor(&output),
    );
    let expected = include_str!("subcube_sum.cu.hip").replace("\r\n", "\n");
    let expected = expected.trim();
    assert_eq!(compile(kernel), expected);
}
...
pub fn sequence_for_loop() {
    ...
    let kernel = sequence_for_loop_kernel::create_dummy_kernel::<HipRuntime>(
        CubeCount::Static(1, 1, 1),
        CubeDim::default(),
        array(&output),
    );
    let expected = include_str!("sequence_for_loop.cu.hip").replace("\r\n", "\n");
    let expected = expected.trim();
    assert_eq!(compile(kernel), expected);
}
...
pub fn unary_bench() {
    ...
    let kernel = execute_unary_kernel::create_dummy_kernel::<f32, HipRuntime>(
        CubeCount::Static(1, 1, 1),
        CubeDim::default(),
        tensor_vec(&lhs, 4),
        tensor_vec(&rhs, 4),
    ...
    let expected = include_str!("unary_bench.cu.hip").replace("\r\n", "\n");
    let expected = expected.trim();
    assert_eq!(compile(kernel), expected);
}
...
pub fn constant_array() {
    ...
    let kernel = constant_array_kernel::create_dummy_kernel::<f32, HipRuntime>(
        CubeCount::Static(1, 1, 1),
        CubeDim::default(),
        tensor_vec(&out, 1),
        data,
    );
    let expected = include_str!("constant_array.cu.hip").replace("\r\n", "\n");
    assert_eq!(compile(kernel), expected);
}
