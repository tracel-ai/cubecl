...
/// the vectorization axis is perpendicular to the last axis.
///
/// This kernel handles the case where memory is laid out such that the unit-stride
/// is not on the last axis, requiring a "gather-and-transpose" pattern
...
fn copy_perpendicular<T: Numeric, N: Size>(
    ...
    // Calculate how many vectorized vectors fit into the last axis shape.
    ...
    let channel_output_stride_elem = output.stride(vectorized_axis);
...
///
/// This is used when the input tensor's memory layout is such that the last axis
/// is not the one with a stride of 1 (the vectorized axis). It optimizes
