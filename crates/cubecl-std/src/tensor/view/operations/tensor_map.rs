        ...
        paste::paste! {
            ...
            impl<T: CubePrimitive> ViewOperationsExpand<T, $coords> for ExpandElementTyped<TensorMap<T, Tiled>> {
                ...
                fn __expand_tensor_map_load_method(
                    ...
                    barrier.[<__expand_tma_load_ $dim d_method>]::<T, Const<1>>(scope, self.clone(), shared, $($var),*);
                }
            }
            ...
            impl<T: CubePrimitive> ViewOperationsMutExpand<T, $coords> for ExpandElementTyped<TensorMap<T, Tiled>> {
                ...
                fn __expand_tensor_map_store_method(
                    ...
                    let ($($var),*) = pos;
                    let ($($var),*) = ($(i32::__expand_cast_from(scope, $var)),*);
                    [<tma_store_ $dim d>]::expand::<T, Const<1>>(scope, shared, self.clone(), $($var),*);
                }
            }
        }
    };
        ...
        paste::paste! {
            ...
            impl<T: CubePrimitive> ViewOperationsExpand<T, $coords> for ExpandElementTyped<TensorMap<T, Im2col>> {
                ...
                fn __expand_tensor_map_load_method(
                    ...
                    barrier.[<__expand_tma_load_im2col_ $dim d_method>]::<T, Const<1>>(scope, self.clone(), shared, $($pos),*, $($offs),*);
                }
            }
        }
    };
    ...
    for ExpandElementTyped<TensorMap<T, Tiled>>
{
    ...
    fn __expand_tensor_map_load_method(
        ...
        let shared: SliceExpand<Line<T, Const<1>>, ReadWrite> =
            shared_memory.__expand_downcast_method(scope);
        let rank = pos.len();
        let pos = &pos;
        match rank {
            1 => {
    ...
    for ExpandElementTyped<TensorMap<T, Tiled>>
{
    ...
    fn __expand_tensor_map_store_method(
        ...
        let shared: SliceExpand<Line<T, Const<1>>, ReadOnly> =
            shared_memory.__expand_downcast_method(scope);
        let rank = pos.len();
        let pos = &pos;
        match rank {
