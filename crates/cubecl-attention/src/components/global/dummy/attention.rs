...
> GlobalAttention<AP> for DummyGlobalAttention<AP, SA>
{
    ...
    fn execute(
        ...
        comptime!(println!("seq_kv_stage: {:?}", seq_kv_stage));

        // let num_stage_iterations = div_ceil(seq_kv, seq_kv_stage.runtime());
        // let seq_kv = 2u32.runtime();
        // let num_stage_iterations = (seq_kv + seq_kv_stage.runtime() - 1) / seq_kv_stage.runtime();
        // let num_stage_iterations = 1u32; //.runtime();

        // for i in 0..num_stage_iterations {
        let out_of_bounds_mask = CubeOption::new_None();
