...
pub type PLElem<PL> = <<PL as PlaneLayout>::RW as RowWise>::E;
...
pub trait PlaneLayout: CubeType {
    ...
    fn get_at_coor(&self, local_row: u32, local_col: u32) -> <Self::RW as RowWise>::E;
    fn scale(&mut self, val: &Self::RW);
    fn scale_and_mask(&mut self, scale: &Self::RW, mask: TileMask);
    fn exp_m_diff(&mut self, m: &Self::RW);
}
