...
impl WgpuStream {
    ...
    pub fn sync(&mut self) -> Pin<Box<dyn Future<Output = ()> + Send + 'static>> {
        // Ensure all pending commands are submitted before creating the fence.
        self.flush();

        // Prefer native completion callbacks when available.
        // wgpu exposes `Queue::on_submitted_work_done` which invokes the callback
        // once all prior submissions have finished. Wrap it into a Future.
