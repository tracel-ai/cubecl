...
impl<'a> ExtendedFeatures<'a> {
    ...
    fn fill_extensions(&mut self, adapter: &vulkan::Adapter, features: Features) {
        ...
        if phys_caps.supports_extension(EXT_SHADER_LONG_VECTOR_NAME) {
            self.extensions.push(EXT_SHADER_LONG_VECTOR_NAME);
            self.long_vector = Some(PhysicalDeviceShaderLongVectorFeaturesEXT::default());
        }
    }
    ...
    fn zero_pointers(&mut self) {
        ...
        if let Some(long_vector) = &mut self.long_vector {
            long_vector.p_next = null_mut();
        }
    }
}
