...
/// Manual implementation of bitcode because it has no way to skip fields
#[cfg(feature = "bitcode")]
mod bitcode_impl {
    use super::*;

    impl bitcode::__private::Encode for Allocator {
        const ENCODE_MIN: usize = <u32>::ENCODE_MIN;
        const ENCODE_MAX: usize = 0usize.saturating_add(<u32>::ENCODE_MAX);
        #[allow(clippy::all)]
        #[cfg_attr(not(debug_assertions), inline(always))]
        fn encode(
            &self,
            encoding: impl bitcode::__private::Encoding,
            writer: &mut impl bitcode::__private::Write,
        ) -> bitcode::__private::Result<()> {
            let next_id = self.next_id.load(Ordering::SeqCst);
            bitcode::__private::optimized_enc!(encoding, writer);
            enc!(next_id, u32);
            end_enc!();
            Ok(())
        }
    ...
    impl bitcode::__private::Decode for Allocator {
        const DECODE_MIN: usize = <u32>::DECODE_MIN;
        const DECODE_MAX: usize = 0usize.saturating_add(<u32>::DECODE_MAX);
        #[allow(clippy::all)]
        #[cfg_attr(not(debug_assertions), inline(always))]
        fn decode(
            encoding: impl bitcode::__private::Encoding,
            reader: &mut impl bitcode::__private::Read,
        ) -> bitcode::__private::Result<Self> {
            bitcode::__private::optimized_dec!(encoding, reader);
            dec!(next_id, u32);
            end_dec!();
            Ok(Self {
                local_mut_pool: Default::default(),
                next_id: Rc::new(AtomicU32::new(next_id)),
            })
        }
    }
}
