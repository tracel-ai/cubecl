...
use super::{
    components::{
        ...
        tile::plane::accelerated::PlaneAcceleratedMatmul,
    },
    kernels::{
        MatmulLaunchError,
        matmul::{
    ...
    match strategy {
        Strategy::Simple(loading_strategy) => match loading_strategy {
            SyncLoadingStrategy::Cyclic => {
                matmul::launch_ref::<R, MP, SimpleAlgorithm<PlaneAcceleratedMatmul>>(
        ...
        Strategy::SimpleBarrier(loading_strategy) => match loading_strategy {
            AsyncLoadingStrategy::Cooperative => {
                matmul::launch_ref::<
                    ...
                    SimpleBarrierAlgorithm<
                        PlaneAcceleratedMatmul,
                        async_full_cooperative::LoadingStrategy,
                    >,
                >(client, lhs, lhs_scale, rhs, rhs_scale, out)
            }
            ...
            AsyncLoadingStrategy::Tma => {
                matmul::matmul_cmma_tma_ref_no_check::<
                    R,
                    MP,
                    SimpleTmaAlgorithm<PlaneAcceleratedMatmul>,
                >(client, lhs, lhs_scale, rhs, rhs_scale, out, (false, false))
            }
        },
        Strategy::DoubleBuffering(loading_strategy) => match loading_strategy {
            SyncBufferLoadingStrategy::Cyclic => {
                matmul::launch_ref::<R, MP, CyclicDoubleBufferingAlgorithm<PlaneAcceleratedMatmul>>(
                    client, lhs, lhs_scale, rhs, rhs_scale, out,
                )
            }
            SyncBufferLoadingStrategy::Tilewise => {
                matmul::launch_ref::<R, MP, TilewiseDoubleBufferingAlgorithm<PlaneAcceleratedMatmul>>(
                    client, lhs, lhs_scale, rhs, rhs_scale, out,
                )
            }
            SyncBufferLoadingStrategy::Hybrid => {
                matmul::launch_ref::<R, MP, HybridDoubleBufferingAlgorithm<PlaneAcceleratedMatmul>>(
                    client, lhs, lhs_scale, rhs, rhs_scale, out,
                )
            }
        },
        Strategy::OrderedDoubleBuffering => {
            matmul::launch_ref::<R, MP, OrderedDoubleBufferingAlgorithm<PlaneAcceleratedMatmul>>(
                client, lhs, lhs_scale, rhs, rhs_scale, out,
            )
        }
        ...
        Strategy::Auto => {
            if let Err(err) = matmul::launch_ref::<R, MP, SimpleAlgorithm<PlaneAcceleratedMatmul>>(
                client, lhs, lhs_scale, rhs, rhs_scale, out,
            ) {
                match err {
