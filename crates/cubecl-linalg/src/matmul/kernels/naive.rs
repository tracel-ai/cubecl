...
pub fn launch<R: Runtime, E: Numeric>(
    ...
    let lhs = if !matches!(lhs_layout, MatrixTensorLayout::Contiguous) {
    ...
    let (rhs_original_shape, rhs) = match rhs_layout {
        MatrixTensorLayout::Contiguous => correct_rhs_layout(rhs),
        ...
        MatrixTensorLayout::HighlyPermuted => correct_rhs_layout(rhs),
    };
