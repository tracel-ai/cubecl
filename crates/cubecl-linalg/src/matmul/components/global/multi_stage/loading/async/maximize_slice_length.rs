use crate::matmul::components::{
    ...
    stage::{DualStage, DualStageExpand, DualStageFormat, StridedTilingLayout},
};
...
impl AsyncBufferLoadingStrategy for MaximizeSliceLengthBufferLoading {
    ...
    fn load_buffer<EG: Numeric, ES: Numeric, G: GlobalConfig, CM: CopyMechanism<ES>>(
        ...
        match stage {
            DualStage::Physical(_) => {
                comptime!(panic!("This loader is for virtual dual stage only"));
            }
            DualStage::Virtual(virtual_dual_stage) => {
                ...
                for nth_slice_local in 0..slices_per_unit {
                    ...
                    let window: Window<EG> =
                        read_view.load_window_in_stage::<G>(nth_slice, ident, config);
                    let mut destination: SliceMut<Line<ES>> =
                        StridedTilingLayout::nth_slice::<ES, G::SmmConfig>(
                            &mut virtual_dual_stage.as_slice_mut(),
                            nth_slice,
                            ident,
