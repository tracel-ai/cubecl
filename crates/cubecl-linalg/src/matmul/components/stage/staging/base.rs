...
impl<ES: Numeric, T: TilingLayout> Stage<ES, T> {
    ...
    pub fn new_with_smem(v_smem: VirtualSharedMemory<ES>) -> Stage<ES, T> {
        Stage::<ES, T> {
            v_smem,
            tiling_layout: PhantomData::<T>,
        }
...
/// Wrapper over the stage to only allow reading tiles
#[derive(CubeType)]
pub struct StageReader<ES: Numeric, T: TilingLayout> {
...
#[cube]
impl<ES: Numeric, T: TilingLayout> StageReader<ES, T> {
    pub fn read_tile<S: StageConfig>(&self, x: u32, y: u32, #[comptime] config: S) -> Tile<ES> {
        self.stage.get_tile::<S>(x, y, self.ident, config)
    }
}
