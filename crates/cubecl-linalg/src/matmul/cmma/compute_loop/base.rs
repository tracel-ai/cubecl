...
pub(crate) fn compute_loop<F: Float, FC: Float>(
    ...
    if comptime_info.compute_loop_order_strategy == 0 {
        BufferInnerComputeLoop::compute_loop(shared_memories, fragments, ids, comptime_info);
    } else {
        BufferOuterComputeLoop::compute_loop(shared_memories, fragments, ids, comptime_info);
    }
}
...
struct BufferOuterComputeLoop {}
struct BufferInnerComputeLoop {}

#[cube]
impl ComputeLoop for BufferOuterComputeLoop {
    fn compute_loop<F: Float, FC: Float>(
        shared_memories: SharedMemories<FC>,
        fragments: &mut Fragments<F, FC>,
        ids: Ids,
...
#[cube]
impl ComputeLoop for BufferInnerComputeLoop {
    fn compute_loop<F: Float, FC: Float>(
        shared_memories: SharedMemories<FC>,
        fragments: &mut Fragments<F, FC>,
        ids: Ids,
