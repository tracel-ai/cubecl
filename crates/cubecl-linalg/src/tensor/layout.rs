...
pub fn matrix_layout(strides: &[usize]) -> MatrixTensorLayout {
    let rank = strides.len();
    if rank <= 1 {
        return MatrixTensorLayout::Contiguous;
    }
    ...
    if row_stride == 0 || col_stride == 0 {
        ...
        return MatrixTensorLayout::HighlyPermuted;
    }
...
mod tests {
    ...
    fn layout_is_contiguous() {
        let strides = &[8, 4, 2, 1];
        assert_eq!(matrix_layout(strides), MatrixTensorLayout::Contiguous);
    }
    ...
    fn layout_is_transposed_only() {
        let strides = &[8, 4, 1, 2];
        if let MatrixTensorLayout::MildlyPermuted {
            transposed,
            batch_swap,
    ...
    fn layout_has_batch_swapped_with_row() {
        let strides = &[8, 2, 4, 1];
        assert_eq!(matrix_layout(strides), MatrixTensorLayout::HighlyPermuted);
    }
