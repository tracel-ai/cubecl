...
fn launch_subgroup<R: Runtime>(
    ...
    unsafe {
        sum_subgroup::launch_unchecked::<f32, R>(
            ...
            ArrayArg::from_raw_parts(input, len, 1),
            ArrayArg::from_raw_parts(output, len, 1),
            client.properties().feature_enabled(cubecl::Feature::Subcube),
            Some(len as u32),
        );
    }
}
    ...
    for kind in [
        ...
        match kind {
            ...
            KernelKind::TraitSum => {
                ...
                // executed.
                if client.properties().feature_enabled(cubecl::Feature::Subcube) {
        ...
        let bytes = client.read(output.clone().binding());
